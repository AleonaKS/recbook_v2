...
def utility_function_recs(name, top_n=5):
    ...
    return top_high_value_books, top_high_interaction_books
        ...
        # # Функция для расчета полезности взаимодействия для каждого пользователя
# def calculate_interaction_value_per_user(reactions_df):
#     # Константы для оценки полезности
#     VIEW_VALUE = 1
#     BOOKMARK_ADDED_VALUE = 2
#     BOOKMARK_REMOVED_PENALTY = -1
#     CART_ADDED_VALUE = 3
...
#     # Функция для расчета полезности в зависимости от времени просмотра
#     def view_value(duration_viewed):
#         # Полезность просмотра перестает возрастать после 3 минут
#         return min(VIEW_VALUE * (duration_viewed / 60), VIEW_VALUE * 3)
#
#     # Функция для расчета полезности добавления в закладки
#     def bookmark_value(date_added):
#         # Полезность возрастает в течение недели, затем растет медленнее в течение 3 месяцев
#         one_week = date_added + timedelta(weeks=1)
#         three_months = date_added + timedelta(weeks=12)
#         now = datetime.now()
#         if now <= one_week:
#             return BOOKMARK_ADDED_VALUE
#         elif one_week < now <= three_months:
#             # Рост полезности замедляется после одной недели
#             return BOOKMARK_ADDED_VALUE * 1.5
#         else:
#             # Полезность убывает после трех месяцев
#             return BOOKMARK_ADDED_VALUE * 0.5
#
#
#     # Функция для расчета полезности в зависимости от оценки
...
#             return 3
    ...
    # Расчет полезности для каждого взаимодействия
    #     reactions_df['interaction_value'] = reactions_df.apply(lambda row:
    #     view_value(row['duration_viewed']) if row['reaction_type'] == 'просмотр' else
    #     bookmark_value(row['date взаимодействия']) if row['reaction_type'] == 'отложено в закладки' else
    #     BOOKMARK_REMOVED_PENALTY if row['reaction_type'] == 'убрано из закладок' else
    #     CART_ADDED_VALUE if row['reaction_type'] == 'отложено в корзину' else
    #     CART_REMOVED_PENALTY if row['reaction_type'] == 'убрано из корзины' else
    #     PURCHASE_VALUE + rating_value(row['evaluation']) if row['reaction_type'] == 'покупка' else
    #     REVIEW_VALUE + rating_value(row['evaluation']) if row['reaction_type'] == 'оставлен отзыв' else 0, axis=1)
    #
    # # Группировка по user_id и расчет суммарной полезности для каждого пользователя
    # user_interaction_values = reactions_df.groupby('user_id')['interaction_value'].sum().reset_index()
    #
    # # Возвращаем датафрейм с user_id и суммарной полезностью
    # return user_interaction_values


#
# def normalize_values(user_interaction_values):
#         min_value = user_interaction_values['interaction_value'].min()
#         max_value = user_interaction_values['interaction_value'].max()
#
#         def normalize_negative(value):
#             return (value - min_value) / (0 - min_value)
#
...
#             return (value - 0) / (max_value - 0)
#
#         user_interaction_values['normalized_value'] = user_interaction_values['interaction_value'].apply(
#             lambda x: normalize_negative(x) if x < 0 else (normalize_positive(x) if x > 0 else 0)
#         )
#         return user_interaction_values
#
#
# user_interaction_values = calculate_interaction_value_per_user(reactions_df)
# normalized_interaction_values = normalize_values(user_interaction_values)
# user_book_matrix = pd.pivot_table(reactions_df, values='interaction_value', index='isbn', columns='user_id', fill_value=0)
#
#
# def get_book_recommendations(reactions_df, user_book_matrix, user_id, top_n=3):
#     user_reactions = reactions_df[reactions_df['user_id'] == user_id]
#
#     high_value_books = user_reactions[user_reactions['reaction_type'].isin(['отложено в корзину', 'покупка', 'оставлен отзыв'])]
#     high_interaction_books = user_reactions[user_reactions['reaction_type'].isin(['просмотр', 'добавлено в закладки', 'отложено в корзину'])]
#
#     high_value_isbn = high_value_books.groupby('isbn')['interaction_value'].sum().reset_index()
#     high_interaction_isbn = high_interaction_books.groupby('isbn')['interaction_value'].sum().reset_index()
#
#     sorted_high_value_isbn = high_value_isbn.sort_values(by='interaction_value', ascending=False)
#     sorted_high_interaction_isbn = high_interaction_isbn.sort_values(by='interaction_value', ascending=False)
#
#     top_high_value_isbn = sorted_high_value_isbn.head(top_n)['isbn'].tolist()
#     top_high_interaction_isbn = sorted_high_interaction_isbn.head(top_n)['isbn'].tolist()
#
#     return top_high_value_isbn, top_high_interaction_isbn
